...
/*   Updated: 2021/05/24 10:04:28 by phbarrad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int e;
	int	*val_med = malloc(sizeof(int) * 16);
	int i = 0;
	int init_med = find_val_med(p);
	//val_med[0] = find_val_med(p);
	//printf("VM = [%d]\n", val_med);
	//while (is_sort(p) == ERROR)
	//{
		//all_cmd_both(p);
		//sswap2(p);
		val_med[i] = find_val_med(p);
		//printf("VM = [%d]\n", val_med);
		while (no_val_med_in_a(p, val_med[i]) == ERROR && p->lena >= 0 && pa_opti_fill(p) == ERROR)
		{
			//printf("opti = [%d]\n", pa_opti_fill(p));
			if (p->pa[p->lena] < val_med[i])
				pb(p, 0);
			else if (p->pa[0] < val_med[i])
				rra(p, 0);
			else
				ra(p, 0);
			if (no_val_med_in_a(p, val_med[i]) == SUCCESS)
			{
				i++;
				val_med[i] = find_val_med(p);
			}
	...
	//	printf("VM-b4 = [%d]\n\n", val_med[e]);
		val_med[e] = next_val_med_pb(p, val_med[e]);
		//printf("\nVM-b2 = [%d]\n", val_med[e]);

		while (no_val_med_in_b(p, val_med[e]) == ERROR && p->lenb >= 0 && pb_opti_fill(p) == ERROR && val_med[e] >= init_med)
		{
			//sswap2(p);
			//printf("VM-b = [%d]\n", val_med[e]);
			if (p->pb[p->lenb] >= val_med[e])
			{
				pa(p, 0);
				val_med[e] = next_val_med_pb(p, val_med[e]);
			}
			else if (pp_g(p->pb, p->lenb, val_med[e]) < pp_d(p->pb, p->lenb, val_med[e]))
				rrb(p, 0);
			else
				rb(p, 0);
			if (no_val_med_in_b(p, val_med[e]) == SUCCESS)
				val_med[e] = next_val_med_pb(p, val_med[e]);
		}
		...
		{
			//all_cmd_both(p);
			val_med[e] = find_val_med_in_b(p);
			//printf("VM-b = [%d]", val_med);
			//sswap(p);
			while (no_val_med_in_b(p, val_med[e]) == ERROR)
			{
				if (p->pb[p->lenb] >= val_med[e])
					pa(p, 0);
				else if (p->pb[0] >= val_med[e])
					rrb(p, 0);
				else
					rb(p, 0);
				//sswap(p);
		...
		//printf("VM-b4 = [%d]\n\n", val_med[i]);
		val_med[e] = next_val_med_pb(p, val_med[i]);
		//printf("\nVM-b2 = [%d]\n", val_med[e]);

		while (no_val_med_in_b(p, val_med[e]) == ERROR && p->lenb >= 0 && pb_opti_fill(p) == ERROR)
		{
			//sswap2(p);
			//printf("VM-b = [%d]\n", val_med[e]);
			if (p->pb[p->lenb] >= val_med[e])
...
/*		int e = i - 2;
	...
	/*while (is_sort(p) == ERROR)
	{
		while (no_val_med_in_b(p, val_med[e]) == ERROR && p->lenb >= 0
		&& pb_opti_fill(p) == ERROR && pa_opti_fill(p) != ERROR)
		{
			sswap2(p);
			printf("VM-b = [%d]\n", val_med[e]);
			if (p->pb[p->lenb] >= val_med[e])
		...
		while (p->lena >= 0 && pa_opti_fill(p) == ERROR)
		{
			//printf("opti = [%d]\n", pa_opti_fill(p));
			if (p->pa[p->lena] <= val_med[e])
				pb(p, 0);
			else if (p->pa[0] <= val_med[e])
				rra(p, 0);
			else
				ra(p, 0);
			if (no_val_med_in_a(p, val_med[e]) == SUCCESS)
			{
				if (e == 0)
					val_med[e] = find_val_med_in_b(p);
				else
