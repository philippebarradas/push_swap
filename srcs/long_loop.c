...
/*   Updated: 2021/05/24 10:36:53 by phbarrad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	long_loop(t_p *p)
{
	...
	//	printf("VM-b4 = [%d]\n\n", val_med[e]);
	val_med = next_val_med_pb(p, val_med);
	//printf("\nVM-b2 = [%d]\n", val_med[e]);
	while (no_val_med_in_b(p, val_med) == ERROR && p->lenb >= 0 && pb_opti_fill(p) == ERROR && val_med >= init_med)
	{
		//sswap2(p);
		//printf("VM-b = [%d]\n", val_med[e]);
		...
		else if (pp_g(p->pb, p->lenb, val_med) <= pp_d(p->pb, p->lenb, val_med))
			rrb(p, 0);
		else
			rb(p, 0);
